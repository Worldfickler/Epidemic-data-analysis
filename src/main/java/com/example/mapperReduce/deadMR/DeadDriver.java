...
public class DeadDriver {
    ...
    public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        //设置jar包路径
        job.setJarByClass(DeadDriver.class);
        //关联mapper和reducer
        job.setMapperClass(DeadMapper.class);
        job.setReducerClass(DeadReducer.class);
        //设置map输出的key，value输出类型
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(DeadWritable.class);
        //设置最终输出的key，value类型
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(DeadWritable.class);
        //设置输入路径和输出路径
        FileInputFormat.setInputPaths(job, new Path("data/province_epidemic.txt"));
        FileOutputFormat.setOutputPath(job, new Path("data/dead_output"));

        FileSystem fs = FileSystem.get(configuration);
        if(fs.exists(new Path("data/province_epidemic.txt"))){
            fs.delete(new Path("data/dead_output"),true);
        }
